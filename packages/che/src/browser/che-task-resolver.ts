...
export class CheTaskResolver implements TaskResolver {
    ...
    async resolveTask(taskConfig: TaskConfiguration): Promise<TaskConfiguration> {
        ...
        if (!cheTaskConfig.target) {
            ...
            resultTarget.workspaceId = cheTaskConfig.target.workspaceId ? cheTaskConfig.target.workspaceId : await this.getWsId();
            resultTarget.machineName = cheTaskConfig.target.machineName ? cheTaskConfig.target.machineName : await this.pickMachine();
        }
