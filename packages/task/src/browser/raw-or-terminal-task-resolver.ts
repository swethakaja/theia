...
import { TerminalTaskConfiguration } from '../common/task-protocol';
...
export class RawOrTerminalTaskResolver implements TaskResolver {
    ...
    async resolveTask(task: TerminalTaskConfiguration): Promise<TerminalTaskConfiguration> {
        const resultTask: TerminalTaskConfiguration = {
            ...
            windowsProcessOptions: task.windowsProcessOptions ? await this.resolveVariablesInOptions(task.windowsProcessOptions) : undefined,
            cwd: await this.variableResolverService.resolve(task.cwd ? task.cwd : '${workspaceFolder}')
        };
        return resultTask;
    }
