...
export class TaskService implements TaskConfigurationClient {
    ...
    async run(taskName: string): Promise<void> {
        ...
        let toRun: TaskConfiguration;
        try {
            toRun = resolver ? await resolver.resolveTask(task) : task;
        } catch (error) {
            this.logger.error(`Error resolving task '${taskName}': ${error}`);
            this.messageService.error(`Error resolving task '${taskName}': ${error}`);
            return;
        }
